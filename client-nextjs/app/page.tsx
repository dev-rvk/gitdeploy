...
export default function Home() {
  ...
  const [subdomain, setSubdomain] = useState<string>(""); // New state for subdomain
  ...
  const handleClickDeploy = useCallback(async () => {
    ...
    try {
      const { data } = await axios.post(`http://localhost:9000/deploy`, {
        gitURL: repoURL,
        slug: subdomain || projectId, // Use subdomain if provided, else projectId
      });
  ...
  const handleSocketIncommingMessage = useCallback((message: string) => {
    ...
    try {
        ...
        const log = parsedMessage.log;
        
        if (log) {
            setLogs((prev) => [...prev, log]);
            logContainerRef.current?.scrollIntoView({ behavior: "smooth" });
        } else {
            console.error("Log field is missing in the incoming message:", message);
        }
    } catch (error) {
